...
export function EventEditor({ event, semester, onCancel, onSave }: Props) {
  ...
  const submit = (formEvent: React.FormEvent) => {
    ...
    if (!name.trim() || teams.length === 0) return;
    if (multiDay) {
  ...
  return (
    <div className="event-dialog-backdrop" role="presentation">
      <form className="event-dialog event-editor" role="dialog" aria-modal="true" aria-labelledby="editor-title" onSubmit={submit}>
        <button className="event-dialog-close" type="button" onClick={onCancel} aria-label="Close editor">×</button>
        <span className="status-tag">Browser-local editor</span>
        <h3 id="editor-title">{event ? "Edit event" : "Add event"}</h3>
        <p className="editor-notice">Changes currently save only in this browser. Shared EXCO editing will replace this storage adapter later.</p>
        <div className="editor-grid">
          <label className="wide"><span>Event name *</span><input value={name} onChange={(e) => setName(e.target.value)} required /></label>
          ...
          <fieldset className="wide"><legend>Managing team *</legend><div className="editor-team-options">{EVENT_TEAMS.map((team) => <label key={team}><input type="checkbox" checked={teams.includes(team)} onChange={() => setTeams(teams.includes(team) ? teams.filter((item) => item !== team) : [...teams, team])} /> {LABELS.teams[team]}</label>)}</div></fieldset>
        ...
        {(teams.length === 0 || validation) && <p className="editor-validation">{validation || "Choose at least one managing team."}</p>}
        <div className="event-dialog-actions"><button type="button" onClick={onCancel}>Cancel</button><button type="submit" className="save">Save event</button></div>
      </form>
    </div>
  );
}
